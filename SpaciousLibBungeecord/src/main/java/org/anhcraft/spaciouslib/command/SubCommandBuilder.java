...
public class SubCommandBuilder extends CommandString{
    ...
    private void init() {
        ...
        setArgErrorMessage(CommandArgument.Type.SERVER,
                "&cCouldn't find that server!");
    ...
    public int hashCode(){
        return new HashCodeBuilder(15, 47)
                ...
                .append(this.hideTypeCommandString).build();
    }
                ...
                for(CommandArgument arg : values.keySet()) {
                    ...
                    switch(getArgumentType(arg)) {
                        ...
                        case INTEGER:
                            ...
                            break;
                        case SERVER:
                            if(!BungeeCord.getInstance().getServers().containsKey(value)) {
                                s.sendMessage(sc.argErrorMessages.get(CommandArgument.Type.SERVER));
                                hasError = true;
                                break argTypeValidator;
                            }
                            break;
                        case ONLINE_PLAYER:
                            if(BungeeCord.getInstance().getPlayer(value) == null) {
