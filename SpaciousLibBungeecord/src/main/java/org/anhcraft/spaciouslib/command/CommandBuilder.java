...
public class CommandBuilder extends CommandString {
     ...
     * @return this object
     */
    public CommandBuilder buildExecutor(Plugin plugin) {
        if(getCommand() != null){
            BungeeCord.getInstance().getPluginManager().unregisterCommand(getCommand());
        }
        PluginCommand c = new PluginCommand(this.name) {
            @Override
            public void execute(CommandSender commandSender, String[] strings) {
                execute_(commandSender, strings);
            }
        ...
        BungeeCord.getInstance().getPluginManager().registerCommand(plugin, c);
        this.command = c;
        return this;
    }
    ...
    private void execute_(CommandSender s, String[] a) {
        // if only contains the root command
        if(getSubCommands().size() == 1){
            try {
