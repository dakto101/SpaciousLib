...
public class NPC_1_8_R1 extends NPCWrapper {
    private EntityPlayer entity;

    public NPC_1_8_R1(NPC npc) {
        this.npc = npc;
        init();
    }
    ...
    private void tablist(boolean add, Player player) {
        new PacketSender(new PacketPlayOutPlayerInfo(add ? EnumPlayerInfoAction.ADD_PLAYER : EnumPlayerInfoAction.REMOVE_PLAYER, this.entity)).sendPlayer(player);
    }

    private void tablist(boolean add, List<Player> players) {
        new PacketSender(new PacketPlayOutPlayerInfo(add ? EnumPlayerInfoAction.ADD_PLAYER : EnumPlayerInfoAction.REMOVE_PLAYER, this.entity)).sendPlayers(players);
    }
}
